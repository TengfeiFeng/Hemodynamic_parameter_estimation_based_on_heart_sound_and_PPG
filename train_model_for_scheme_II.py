...
def load_data_1(input_directory):
# load data location for subject 1 
    '''
    input_directory: the location for records of subject 1
    '''
    data = []
...
def load_data_2(input_directory):
# load data location for subject 2
    '''
    input_directory: the location for records of subject 2
    '''
    data = []
    root_path = 'data/subject_2'
    for folder in range(len(input_directory)):
        tmp_folder = os.path.join(root_path,input_directory[folder])
        for f in os.listdir(tmp_folder):
            if os.path.isfile(os.path.join(tmp_folder, f)) and not f.lower().startswith('.') and f.lower().endswith('mat'):
...
if __name__ == '__main__':
    # give an example for evaluating on subject 2 trining model with subject 1
    TRAIN_DATA_PATH = 'data/subject_1'
    folders = os.listdir(TRAIN_DATA_PATH)
    np.random.shuffle(folders)
    np.save('sub1_shuffle_folders.npy',folders)
    train_folders = np.load('sub1_shuffle_folders.npy')
    folders = []
    TEST_DATA_PATH = 'data/subject_2' 
    folders = os.listdir(TEST_DATA_PATH)
    np.random.shuffle(folders)
    np.save('sub2_shuffle_folders.npy',folders)
    folders = np.load('sub2_shuffle_folders.npy')
    kf = KFold(n_splits=2)
    i = 1
    train_index=np.array(range(14))
    for val_index,test_index in kf.split(folders):
        train_data= []
        val_data = []
        print('train_index', train_index, 'val_index',val_index,'test_index', test_index)
        train_folders = train_folders[train_index]
        val_folders = folders[val_index]
        test_folders = folders[test_index]
        train_data = load_data_1(train_folders)
        val_data = load_data_2(val_folders)
        test_data = load_data_2(test_folders)
        
        '''
        use follow code to select one record of target subject to train the model 
        for Disussion on individual difference in paper 4C
        '''
#        train_folders2.append(folders[val_index[1]])
#        train_folders2 = folders[train_index2]
#        val_index = np.delete(val_index,1)
#        val_data = load_data_2(val_folders)
#        train_data2 = load_data_2(train_folders2)
#        train_data = np.hstack([train_data,train_data2])

        filepath = 'saved_model_for_scheme-II/'+str(i)
        i+=1
 ...
 # then load the best validation models with smallest mae values to evaluate on the corresponding hidden test data, respectively
    ...
    for i in range(1,3):
        ...
        test_folders = folders[(i-1)*7:i*7]
        test_data = load_data_2(test_folders)
